...
import { addFirestoreItem, getFirestoreItems } from '@/lib/firestore-items';
...
export async function GET() {
  try {
    const items = await getFirestoreItems();
...
export async function POST(req: Request) {
  try {
    ...
    const newItem: Item = {
      ...
      reporterName:
        body.reporterName || 'Anonymous Reporter',

      reporterContact:
        body.reporterContact || '',
    };

    const saved = await addFirestoreItem(newItem);
